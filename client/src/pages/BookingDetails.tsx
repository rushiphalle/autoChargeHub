...
const BookingDetails: React.FC = () => {
  ...
  const handleStatusUpdate = async (newStatus: string) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/bookings/${booking._id}/status`, { status: newStatus });
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      setActionLoading(true);
      await axios.put(`http://localhost:5000/api/bookings/${booking._id}/cancel`);
      await fetchBookingDetails();
    } catch (err: any) {
  ...
  const handleAddReview = async (rating: number, review: string) => {
    ...
    try {
      setActionLoading(true);
      await axios.post(`http://localhost:5000/api/bookings/${booking._id}/review`, { rating, review });
      await fetchBookingDetails();
    } catch (err: any) {
